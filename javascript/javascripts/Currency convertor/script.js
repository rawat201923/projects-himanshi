...
// object includes key-value pairs of currency codes and their respective rates against
...
//api function to get exchange rate  using API
const getExchangeRate = async() => {
    const amount = parseFloat(fromAmountElement.value);
    const fromCurrency = fromCurrencyElement.value;
    const toCurrency = toCurrencyElement.value;

    //FETCH DATA FROM API   https://api.fastforex.io/fetch-all?api_key=206dd7f8a3-f60d10f8df-s9lxp7
     const response = fetch(`https://api.exchangerate-api.com/v4/latest/${fromCurrency}`);
